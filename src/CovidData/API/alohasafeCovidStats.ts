...
const toCovidData = (stateData: StateCovidData): CovidData => {
  ...
  const timeseries = compact(State.case_counts_last14days).map(toCovidDatum)
  ...
  return {
    ...
    state: "HI",
    population: 1415872,
    metrics: { testPositivityRatio: State.positive_test_rate_last7days, caseDensity: State.avg_cases_last7days },
